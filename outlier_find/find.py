...
def find_outliers_and_mark(df: pd.DataFrame, datetime_col: str = "time_str"):
    ...
    h_idx = settings.get("h_location", 3)
    ...
    for ts in dataset.index:
        ...
        if temp_fault.loc[ts]:
            reasons.append("온도 센서 이상 또는 급격한 변화(비정상 패턴)")
        if hum_fault.loc[ts]:
            reasons.append("습도 센서 이상 또는 급격한 변화(비정상 패턴)")
        if light_outlier.loc[ts]:
            reasons.append("광 센서 이상(물리 범위/패턴/5400 이상)")
